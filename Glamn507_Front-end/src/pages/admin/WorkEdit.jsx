...
function WorkEdit() {
    ...
    useEffect(() => {
        fetch(`https://glamn507.com/api/work/${id}`)
            .then(res => res.json())
            .then(data => {
                const imagesWithUrl = (data.images || []).map(img => {
                    return `https://glamn507-portfolio-storage.s3.ap-northeast-2.amazonaws.com/${img}`;
                });
                setFormData(prev => ({
                    ...prev,
                    ...data,
        ...
        fetch("https://glamn507.com/api/category")
            .then(res => res.json())
        ...
        try {
            ...
            const cleanImageUrls = formData.images.map(img => {
                if (typeof img === "string") {
                    return img.split("https://glamn507-portfolio-storage.s3.ap-northeast-2.amazonaws.com/")[1]; // 상대 경로만 추출
                }
            ...
            const response = await fetch(`https://glamn507.com/api/work/${id}`, {
                method: "PUT",
                headers: {
    ...
    const baseUrl = "https://glamn507-portfolio-storage.s3.ap-northeast-2.amazonaws.com/";
